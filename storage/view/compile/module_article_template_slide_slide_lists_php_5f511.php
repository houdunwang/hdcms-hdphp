...
<div>
    ...
    <ul class="nav nav-tabs" role="tablist">
        ...
        <li><a href="<?php echo  url('slide.post') ?>">添加图片</a></li>
    </ul>
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                <table class="table table-hover">
                    ...
                    <tbody>
                    <?php if(is_array($data) || is_object($data)){foreach ($data as $d){?>
                        <tr>
                            <td>
                                <input type="text" class="form-control input-sm"
                                       name="slide[<?php echo htmlspecialchars($d['id'])?>]"
                                       value="<?php echo htmlspecialchars($d['displayorder'])?>">
                            </td>
                            <td>
                                <img onclick="hdcms.preview('<?php echo htmlspecialchars($d['thumb'])?>')"
                                     src="<?php echo htmlspecialchars(pic($d['thumb']))?>"
                                     style="height:45px;border:solid 1px #dcdcdc;">
                            </td>
                            <td><?php echo htmlspecialchars($d['title'])?></td>
                            <td>
                                <div class="btn-group">
                                    <a href="<?php echo  url('slide.post')?>&id={{$d['id'] !!}"
                                       class="btn btn-default btn-sm">编辑</a>
                                    <a href="javascript:del(<?php echo htmlspecialchars($d['id'])?>)"
        ...
        function del(id) {
            require(['hdjs'], function (hdjs) {
                hdjs.confirm('确定幻灯图片吗,删除后将不可以恢复?', function () {
                    url = "<?php echo  url('slide.remove') ?>&id=" + id;
